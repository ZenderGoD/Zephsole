...
async function requireProjectAccess(ctx: MutationCtx | QueryCtx, projectId: Id<"projects">) {
...
export const createProject = mutation({
  args: { name: v.string(), workshopId: v.id("workshops") },
  handler: async (ctx, args) => {
    const userId = await requireAuthUserId(ctx);
    await requireWorkshopMember(ctx, args.workshopId);
...
export const dequeueMessage = mutation({
  ...
  handler: async (ctx, args) => {
    const { project } = await requireProjectAccess(ctx, args.projectId);
    if (!project.messageQueue || project.messageQueue.length === 0) return null;
...
export const getProjectBySlug = query({
  ...
  handler: async (ctx, args) => {
    ...
    const membership = await ctx.db
      .query("workshopMembers")
      .withIndex("by_workshop_user", (q) => 
        q.eq("workshopId", workshop._id).eq("userId", userId)
      )
      .first();
...
export const renameClassification = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!classification) throw new ConvexError("CLASSIFICATION_NOT_FOUND");
    await requireWorkshopMember(ctx, classification.workshopId);
