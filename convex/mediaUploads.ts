...
const MAX_UPLOAD_BYTES = 20 * 1024 * 1024; // Limit uploads to keep R2 costs predictable
...
export const getUploadUrl = action({
  ...
  handler: async (_ctx, args) => {
    ...
    if (!args.contentType.startsWith("image/")) {
      throw new Error("Only image uploads are allowed right now");
    }
    ...
    const objectKey = `projects/${args.projectId}/${Date.now()}-${randomUUID()}-${safeName}`;
